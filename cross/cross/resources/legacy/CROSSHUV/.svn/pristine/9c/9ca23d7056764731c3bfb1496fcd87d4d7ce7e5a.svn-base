...
//	*
	...
	*	@param array $rcData (Arreglo con la data del archivo adjunto)
	*	@return string $osbresult (Cadena con el codigo de resultado)
	*   @date 07-Apr-2015 14:43
	*   @location Cali-Colombia
	*/
	function SendGenericEmail($sbEmaidesdes, $sbEmaiparas, $sbEmaiasuntos, $sbEmaitextos, $sbFoemcodigos, $rcData) {
		...
		//se valida que los campos necesario para el emailesten cargados
		if ($sbEmaidesdes && $sbEmaiparas && $sbEmaiasuntos && $sbEmaitextos) {				

			$objService = Application :: loadServices("Data_type");
			
			if($rcData["ordenumeros"]){
