...
function smarty_function_viewListadoSeguimientos($params, & $smarty)
{
	...
	if ($rcData) 
	{
		...
		//Pinta la tabla
		//No. de caso, fecha de registro, Vencimiento de compromisos, Infractor, tipo, Clasificacion, Subclasificacion, Dependencia, Localizacion, Listado de compromisos, Estado del compromiso
		...
		$rcFilaLabels = array($rclabels["ordenumeros"]["label"],$rclabels["ordefecregd"]["label"],$rclabels["accofecrevn"]["label"],$rclabels["tiorcodigos"]["label"],$rclabels["evencodigos"]["label"],$rclabels["causcodigos"]["label"],$rclabels["orgacodigos"]["label"],$rclabels["locacodigos"]["label"],$rclabels["compcodigos"]["label"],$rclabels["accoobservas"]["label"],$rclabels["accoactivas"]["label"]);
...
/*			$rcConsult[] = array($rcRow["ordenumeros"],$objDate->fncformatofechahora($rcRow["ordefecregd"]),$objDate->fncformatofechahora($rcRow["accofecrevn"]),
			...
			$rcConsult[] = array($rcRow["ordenumeros"],$objDate->fncformatofechahora($rcRow["ordefecregd"]),$objDate->fncformatofechahora($rcRow["accofecrevn"]),
						   $rcTipoCaso[$rcRow["tiorcodigos"]],$rcEventos[$rcRow["tiorcodigos"]][$rcRow["evencodigos"]],$rcCausas[$rcRow["tiorcodigos"]][$rcRow["evencodigos"]][$rcRow["causcodigos"]],
						   $rcDependencias[$rcRow["orgacodigos"]],$rcLocalidades[$rcRow["locacodigos"]],$rcCompromisos[$rcRow["compcodigos"]],$rcRow["accoobservas"],$rcEstadosComp[$rcRow["accoactivas"]]); 
		...
		foreach ($rcData as $nuCont => $rcRow)
		{
			...
			//S�lo casos finalizados pueden tener seguimientos
