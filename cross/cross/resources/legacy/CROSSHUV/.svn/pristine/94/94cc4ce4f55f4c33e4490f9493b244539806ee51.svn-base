...
function smarty_function_viewconsolidado($params, & $smarty) {
	...
	if ($ordefecdiginin && $ordefecdigfinn) {
		...
		$gateway->setFechasDigitacion($ordefecdiginin,$ordefecdigfinn);
	}
...
function fncPaintReporte($sbPeriodo, $sbDependencia, $sbnameDepend, $rcDatos, $rcTipoReq, $rcReqEstado, $rcEstadoReq, $rcClasifica, $rcSubClasificacion, $rcMedios, $rcLabels) {
	...
	//CLASIFICACION DE DARN POR TIPO
	$sbCadena .= fncReporteClasificacion($rcDatos,$rcClasifica,$rcSubClasificacion,$rclabels,$objGraph);
	...
	$sbCadena .= fncReporteTiposReqTerminos($rcDatos, $rcTipoReq, $rclabels, $objGraph);
...
function fncReporteTiposReqTerminos($rcDatos, $rcTipoReq, $rclabels, $objGraph) {
	$sbCadena .= "<table border=0>";
	$sbCadena .= "<tr>";
	$sbCadena .= "  <td class=\"titulofila\">".$rclabels["titulo9"]["label"]."</td>";
	$sbCadena .= "</tr>";
	$sbCadena .= "<tr>";
	$sbCadena .= "  <td class=\"piedefoto\">".fncPaintTipoCasoTerminos($rcDatos, $rcTipoReq, $rclabels, $rcDatosReporteTipos);
	$sbCadena .= "      <br><p>".$rclabels["pieatencion"]["label"]."</td>";

	if (is_array($rcDatosReporteTipos)) {
		//EXCEL
		if (is_array($rcDatosReporteTipos[2])) {
			array_unshift($rcDatosReporteTipos[2], array ($rclabels["tiporeq"]["label"],$rclabels["atencion"]["label"], $rclabels["no"]["label"], $rclabels["acumulado"]["label"]));
			array_unshift($rcDatosReporteTipos[2], array ($rclabels["titulo9"]["label"]));
			$sbCadena .= getExcelBook($rcDatosReporteTipos[2], $rclabels);
		}
		//IMAGEN
		$objGraph->nuWidth = 500;
...
function fncPaintTipoCasoTerminos($rcDatos, $rcTipoReq, $rcLabels, & $rcDatosReporteEstados) 
{
		...
		foreach ($rcDatos as $rcOrden) 
		{
			...
			if ($rcOrden['ordefecfinad']) 
			{
				...
				if ($rcOrden['ordefecfinad'] > $rcOrden['ordefecvend'])
					$rcReporte[$rcOrden["tiornombres"]][$rcLabels["fuera"]["label"]]++;
					//$rcNoFechatermino ++;
				else
					$rcReporte[$rcOrden["tiornombres"]][$rcLabels["dentro"]["label"]]++;
					//$rcSiFechatermino[$rcOrden["tiornombres"]] ++;
			} else 
			{
				//Ver si es con horas
				$hour = $serviceDate->getHour2DateInSecs($rcOrden['ordefecvend']);
				...
				if ($nuDia < $rcOrden['ordefecvend'])
					$rcReporte[$rcOrden["tiornombres"]][$rcLabels["pendnovenci"]["label"]]++;
					//$rcPendiente[$rcOrden["tiornombres"]] ++;
				else
					$rcReporte[$rcOrden["tiornombres"]][$rcLabels["pendvenci"]["label"]]++;
					//$rcPendienteFuera[$rcOrden["tiornombres"]] ++;
			}
		}
	}
	$nuCont = 0;
	...
	if(is_array($rcReporte)) {
		foreach ($rcReporte as $tiornombres=>$rcTermino) {
			foreach ($rcTermino as $label=>$valorTermino) {
				$sbCadena .= "<tr>";
				$sbCadena .= "<td align=center>". (++ $nuCont)."</td>";
				$sbCadena .= "<td align=left>".$tiornombres."</td>";
				$sbCadena .= "<td align=left>".$label."</td>";
				$sbCadena .= "<td align=center>".$valorTermino."</td>";
				$nuPorcentaje = fnuPorcentaje($valorTermino, $nuReg);
				$sbCadena .= "<td align=center>".$nuPorcentaje."</td>";
				$sbCadena .= "</tr>";
			
				$rcDatosReporteEstados[0][$nuCont] = $nuCont;
				$rcDatosReporteEstados[1][$nuCont] = $valorTermino;
				$rcDatosReporteEstados[2][$nuCont] = array ($tiornombres ,$label, $valorTermino, $nuPorcentaje);
			}
		}
	}

	$sbCadena .= "<tr>";
	$sbCadena .= "<td colspan=2><b>&nbsp;</b></td>";
	$sbCadena .= "<td align=right>".$rcLabels["total"]["label"]."</td>";
...
function fncPaintClasificacion($rcDatos, $rcClasifica, $rcSubClasificacion, $rcLabels,&$rcDatosReporteClasifica) 
{
	...
	$rcDatosReporteClasifica[2][] = array ($rcLabels["total"]["label"], $nuSubSubTotal, 100);
...
function getExcelBook($rcDatos, $rclabels) {
	...
	$sbTmp .= Application::getConstant("SLASH");
	...
	return "<td class='piedefoto'><a href=javascript:abrirPdf('".$sbPath."');><img border=0 src='web/images/generar_excel.gif' alt='".$rclabels['excel']['label']."'></a></td>";
}

function fncReporteClasificacion($rcDatos,$rcClasifica,$rcSubClasificacion,$rclabels,$objGraph)
{
	$sbCadena .= "<table border=0>";
	$sbCadena .= "<tr>";
	...
	$sbCadena .= "</tr>";
	$sbCadena .= "<tr>";
	$sbCadena .= "  <td class=\"piedefoto\">".fncPaintClasificacion($rcDatos,$rcClasifica,$rcSubClasificacion,$rclabels,$rcDatosReporteTipos);
