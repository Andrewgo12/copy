...
class FeCuSolicitanteManager{
	function FeCuSolicitanteManager(){
		...
		ProvitionalLog::write_log("Object Gateway");
		ProvitionalLog::write_log(print_r($this->objGateway, true));
	}
	...
	function addSolicitante(){
		...
		if($rcData && is_array($rcData)){
			...
			ProvitionalLog::write_log("PARAMS MANAGER");
			ProvitionalLog::write_log(print_r($rcContacto, true));
			ProvitionalLog::write_log(print_r($rcParams, true));
