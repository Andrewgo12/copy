...
Class FePrCmdShowByIdAuth {
	...
	function execute() {
		...
		if (($auth__authusernams != NULL) && ($auth__authusernams != "")) {
			...
			$rcSchemas = $authSchema->getSchemasUser($auth_data[0]["authusernams"]);

			if(is_array($rcSchemas)){
				if ($auth__authusernams == Application::getConstant('ADMIN_USER')){
					$_REQUEST["auth__schecodigon"] = $rcSchemas[0]['schecodigon'];
				}else{
					$_REQUEST["auth__schecodigon"] = $this->getIds($rcSchemas);
				}
			}
			...
			$_REQUEST["auth__schecodigon"] = WebSession :: getProperty("auth__schecodigon");
		...
		WebSession :: unsetProperty("auth__schecodigon");
	...
	function getIds($rcSchemas)
	{
		settype($rcReturn,"array");
		
		foreach ($rcSchemas as $nuKey=>$rcRow)
			$rcReturn[$nuKey] = $rcRow['schecodigon'];
		return $rcReturn;
	}
}
