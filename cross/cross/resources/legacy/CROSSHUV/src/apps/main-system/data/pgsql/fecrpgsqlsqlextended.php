...
class FeCrPgsqlSqlExtended {
	...
	function getListadoOrdenes($rcParams, $rcParamsDate, $rcView, $rcOrder, $children = null, $tareacc = null) {
		...
		settype($sbTablaGrupoInteres,"string");

		//estado inactivo
		$sbEstateI = Application :: getConstant("REG_INACT");

		//Se agrega variable con consulta que se utilizará como tabla para traer el grupo de interés
		$sbTablaGrupoInteres = '(SELECT "ordengruposinteres"."ordenumeros","gruposinteres"."grincodigos","gruposinteres"."grinnombres"
 								FROM "ordengruposinteres","gruposinteres" 
 								WHERE "ordengruposinteres"."grincodigos"="gruposinteres"."grincodigos") AS "ordengruposinteres" ';
		...
		$rcLeftJoin["view_solicitante.contcodigon"] = array ("view" => '"view_solicitante"."contnombres" as "contidentis"', "where" => '"ordenempresa"."contidentis"="view_solicitante"."solicodigos"', "table" => 'view_solicitante',"source"=>'"ordenempresa"');
		$rcLeftJoin["view_solicitante.cliecodigos"] = array ("view" => '(COALESCE("view_solicitante"."cliecodigos", \'\') || \' -- \' || COALESCE("view_solicitante"."clienombres", \'\')) as "cliecodigos"', "where" => '"ordenempresa"."contidentis"="view_solicitante"."solicodigos"', "table" => 'view_solicitante',"source"=>'"ordenempresa"');
		...
		$rcLeftJoin["ordengruposinteres.grincodigos"] = array ("view" => '"ordengruposinteres"."grinnombres" AS "grincodigos"', "where" => '"ordenempresa"."ordenumeros"="ordengruposinteres"."ordenumeros"', "table" => $sbTablaGrupoInteres,"source"=>'"ordenempresa"');

		$rcTables['"orden"'] = null;
		$rcTables['"ordenempresa"'] = null;

		//Ingresa a este bloque si activas un parámetro en la búsqueda del listado, o sea si llenas una variable del lista (no con darle click en en checkbox)
					...
					if($sbFlag){
						if($rcRules[$field]["table"]){
							...
							if($rcLeftJoin[$field]["table"] && !$rcTablesJ[$rcLeftJoin[$field]["table"]]){
								if($field=="ordengruposinteres.grincodigos"){
		...
		if(is_array($rcView) && $rcView){
			foreach ($rcView as $field){
				...
				//Se comienza a analizar los JOINs
				if(array_key_exists($field,$rcLeftJoin)){
					$rcNewView[] = $rcLeftJoin[$field]["view"];

					if($rcLeftJoin[$field]["table"] && !$rcTablesJ[$rcLeftJoin[$field]["table"]]){
						//Meto manejo de la doble comilla para la tabla de ordengruposinteres que construi como un Select
						if($field=="ordengruposinteres.grincodigos"){
							$sbLeft = ' LEFT JOIN '.$rcLeftJoin[$field]["table"].' ON('.$rcLeftJoin[$field]["where"].') ';
						}
						else{
		...
		//MODIFICACION AGOSTO 29 - PAGINACIÓN LISTADO DE CASOS
		//Necesitamos calcular cuántos registros hay
	...
	function getSqlConsult($id,$ircParam=null){
		...
		switch($id){
			...
			case "orden":
				...
				//POR SOLICITUD DE CAROLINA, YA NO HACEMOS ESTA EXCEPCI�N
	 ...
	 * @modified
	 * Se a�ade el parametro que permite definir que tipos de caso deben ser excluidos
	 * @author freina<freina@parquesoft.com>
	...
	/**
	 * Consulta caso específico por número de orden
	 * @param string $ordenumeros
	 * @return array
	 */
	function getSolReqByOrdenumeros($ordenumeros){
		$sql = 'SELECT '.
                    '"ordenempresa"."ordenumeros", '.
                    '"ordenempresa"."oremsolucios" '.
                'FROM "ordenempresa" '.
                'WHERE '.
                    '"ordenempresa"."ordenumeros" = \''.addslashes($ordenumeros).'\' AND '.
                    '"ordenempresa"."oremsolucios" IS NOT NULL '.
                'ORDER BY "ordenempresa"."ordenumeros"';
