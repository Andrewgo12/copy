...
class FeCrPgsqlOrdenempresaExtended {
	...
	function addOrdenempresaSql($ordenumeros, $contidentis,$priocodigos, $tiorcodigos,$evencodigos, $causcodigos, $orgacodigos, $merecodigos, 
	                            $locacodigos, $oremradicas,$oremsolucios,$infrcodigos,$paciindentis,$sesocodigos,$couscodigos,$ipsecodigos) {
	...
	function updateOrdenempresaSql($ordenumeros, $contidentis, $priocodigos,$tiorcodigos, $evencodigos, $causcodigos, $orgacodigos, $merecodigos, 
	                               $locacodigos,$oremradicas,$infrcodigos,$oremsolucios,$paciindentis,$sesocodigos,$couscodigos,$ipsecodigos) {
	...
	function getByIdOrdenempresajoin($ordenumeros) {
		...
		if($rcOrden["contidentis"]){
			...
			if(is_array($rcTmp) && $rcTmp){
				...
				$rcContact = $objGateway->getByIdContacto($rcTmp["contcodigon"]);  //Se inactiva la linea siguiente y se vuelve a activar esta, ya que grupos de interes no es del contacto
    ...
    */
	...
	/**
    * Copyright 2020 FullEngine
    * 
    * Consulta los req de un paciente
    * @author cazapata<cazapata@fullengine.com>
    * @param string $sbOrdenumeros Número de la orden
    * @return array con el registro
    * @date 4-Noviembre-2020 12:40:00 am
    * @location Cali-Colombia
    */
    function getReqByGrupoInteres($sbOrdenumeros){
    	settype($sbSql, "string");
        $sbSql='SELECT "gruposinteres"."grinnombres" FROM "ordenempresa","ordengruposinteres","gruposinteres" 
				WHERE "ordenempresa"."ordenumeros"=\''.$sbOrdenumeros.'\' 
					   AND "ordenempresa"."ordenumeros"="ordengruposinteres"."ordenumeros" 
					   AND "ordengruposinteres"."grincodigos"="gruposinteres"."grincodigos"';
        $this->objdb->fncadoselect($sbSql,FETCH_ASSOC);
        return $this->objdb->rcresult;
    }
