			...
			if($this->nuCant && $this->sbResult){
				...
				if(is_array($rcData) && $rcData){
					...
					foreach($rcData as $rcTmp){
						...
						//Nuevo ajuste de Corpocaldas a HUV
						$this->ordefecregd = $rcTmp["ordefecregd"];
						$this->ordefecvend = $rcTmp["ordefecvend"];
						
						//obtiene la cantidad de dias que han pasado para la orden
						$this->_getDiasCaso();
						//si cumple debe generarse alerta
						$nuDays = $this->nuDiasCaso;
				...
				if(!empty($this->rcOrden) && is_array($this->rcOrden) && $this->rcOrden){
					$this->_getInfoEmail();
					$this->_sendEmail();
				}
			}
	...
	function _getEmailEmp(){
		...
		if(is_array($rcDep) && $rcDep){
			foreach($rcDep as $sbIndex=>$rcTmp){
				...
				$rcParams["dir_origen"] = $this->emailComp;
//				$rcParams["dir_destino"] = $rcTmp["email"];          //Activar para producción
				$rcParams["dir_destino"] = "cross@huv.gov.co";       //Activar para pruebas
				$rcParams["usuario"] = $this->rcUser["username"];
				$rcParams["adjunto"] = null;
	...
	function _getDiasCaso(){
		...
		$this->nuDiasCaso = 0;
		$sbOrdenumeros = $this->ordenumeros;
		$sbOrdefecregd = $this->ordefecregd;
		$sbOrdefecvend = $this->ordefecvend;

		if($sbOrdenumeros){
           //Ajuste alertas para HUV
		   $objService = Application :: loadServices("General"); //llamado a la funcion
		   $rcTime = $objService->get_Time_Progress($sbOrdefecregd,$sbOrdefecvend);
		
			if(is_array($rcTime) && $rcTime){
				$this->nuDiasCaso = $rcTime["restantes"];
			}
		}
		return;
	}
	...
	function _loadApplication($sbModule){
		...
		new Application($sbModule, $sbPath,true);
		return true;
	}
	/**
	 * Copyright 2017 FullEngine
