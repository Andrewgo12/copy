...
function LoadSelect(sqlId,params,rcObject,rtnObject,CleanDescendants,debug)
{
    ...
    jsrsExecute("rs/jsrs.php", this.RtnValue, "AutoCompletar::Select", Array(sqlId,params),false);
 	return ;
 }
...
function RtnValue(sbValue){
	...
	//se carga el combo
 	...
 	for(nuCont=0;nuCont<nuCant;nuCont++)
 	{
 		...
 		rcDuplas[1] = decodeURI(rcDuplas[1].replace(/\+/g, " "));
...
* @param object rtnObject (Objeto tipo select donde se pintara el resultado)
* @param boolean debug para pintar el debug, por defecto en false
* @param string template plantilla 'nombretabla_nombrecampo_nombredescriptor' que se va acargar en el select
*@param string CleanDescendants String with the name of the select descendants objects 
* 
*/
function LoadSelect_lang(sqlId,params,rcObject,rtnObject,template,CleanDescendants,debug)
{
	homefield = rcObject[0];
	rtnfield = rtnObject;
    var rcTriadas = new Array();
    ...
    //Envia a ejecutar en el RS
    jsrsExecute("rs/jsrs.php", this.RtnValue, "AutoCompletar::Select_lang", Array(sqlId,params,template),debug);
 	return ;
 }
