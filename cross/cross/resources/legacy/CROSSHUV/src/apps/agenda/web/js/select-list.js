...
function addGroup() 
{
	...
	var at = form.multiple1.length -1;
	var td = form.multiple2.length -1;
	var groups = "x";

	//build array of groups
	for (td; td > -1; td--) 
	{
		groups = groups + "," + form.multiple2.options[td].value + ","
	}

	//Pull selected resources and add them to list
	for (at; at > -1; at--)
	{
		if (form.multiple1.options[at].selected && groups.indexOf( "," + form.multiple1.options[at].value + "," ) == -1)
		{
			t = form.multiple2.length
			opt = new Option( form.multiple1.options[at].text, form.multiple1.options[at].value );
			form.candidato__candcargoins2.value += "_"+form.multiple1.options[at].value;
			form.multiple2.options[t] = opt
		}
	}
}
...
function removeGroup() {
	var form = document.forms[0];
	td = form.multiple2.length -1;

	for (td; td > -1; td--)
	{
		if (form.multiple2.options[td].selected)
		{
			form.multiple2.options[td] = null;
			//form.candidato__candcargoins2.value -= "_"+form.multiple2.options[td].value;
		}
	}
}

function viewParamsSchema(sbValue)
{
	var url = "index.php?action=FeGeCmdDefaultParametros&schema="+sbValue;
	location = url;
}

function validarNulosMultiples()
{
	nuTam = document.forms[0].length;
	for(nuCont=0;nuCont<nuTam;nuCont++)
	{
		if(document.forms[0].elements[nuCont].type=='select-multiple')
		{
			nuTamObj = document.forms[0].elements[nuCont].options.length;
			document.forms[0].elements[nuCont].options[nuTamObj] = new Option("" ,"");
			document.forms[0].elements[nuCont].options[nuTamObj].selected = true;
		}
	}
	return true;
}

function HideAndSeek(sbValue)
{
	document.forms[0].ordenumeros.options.length = 0;
	add_record( document.forms[0].ordenumeros,'','');
	
	var sbLista = document.forms[0].ordenumeroslist.value;
	var rcLista = sbLista.split(",");
	var nuSize = rcLista.length;
	var nuStrlen = sbValue.length;
	
	for(nuCont=0;nuCont<nuSize;nuCont++)
		if(rcLista[nuCont].substr(1,nuStrlen) == sbValue)
		{
			var rcOption = rcLista[nuCont].split("'");
			add_record( document.forms[0].ordenumeros, rcOption[1],rcOption[1]);
		}
}