...
function smarty_function_viewfichaordweb($params, & $smarty) {
	...
	$rcHtml[] = paintCasesFiles($orden__ordenumeros, $rclabels);
	...
	if (is_array($rcActas) && $rcActas && $rcData["ordefecfinad"]) {
	    
	    /**
	     * se crea la regla que determina el acta a presentar
	     */
	    $nuCant = sizeof($rcActas);
	    
	    if ($nuCant > 1) {
	        $rcActas = array(0=>$rcActas[$nuCant - 1]);
	    }
		...
		foreach ($rcActas as $rcTmpActa) {
			...
			if (is_array($rcAtenc) && $rcAtenc) {
			    
			    /**
			     * se crea la regla que determina la ultima atencion a presentar
			     */
			    $nuCantAt = sizeof($rcAtenc);
			    
			    if ($nuCantAt > 1) {
			        $rcAtenc = array(0=>$rcActas[$nuCantAt - 1]);
			    }
				...
				foreach ($rcAtenc as $rcTmpAtenc) {
					...
					$rcHtml[] = "<tr><td class='piedefoto'>";
					$rcHtml[] = "<table align='left' width='100%'>";
					$rcHtml[] = "<tr><th><div align='left'>".$rclabels["respuesta"]["label"]."</div></th></tr>";
...
function paintAttentionFiles($sbAcemnumeros,$rcLabels){
	...
	if($sbAcemnumeros){
		...
		if($rcResult){
			...
			$rcA["files"]=$sbTmp;
			$sbHtml .= "<tr><th>".$rcLabels["anexos_ar"]["label"]."</th></tr><tr><td class='piedefoto'>";
