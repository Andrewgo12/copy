...
class detallado {
	...
	function detallado($reporte,$date1,$date2,$rcLabels,$periodo,$tipo,$dep,$rcEstadosCompromiso=false,$seguimiento='',$ordefecdiginin,$ordefecdigfinn){
        ...
        $this->gateway = Application::getDataGateway('detallado');
    ...
    */
    function makeReport(){
        ...
        switch($this->reporte){
            case 'tipos': 
            	$sbHtml = $this->makeByAllTipo();
            	break;
            case 'estado': 
            	$sbHtml = $this->makeByEstado();
            	break;
            case 'tipo': 
            	$sbHtml = $this->makeByTipo();
            	break;
            case 'desagregado': 
            	$sbHtml = $this->makeByDependenciaDesagregado();
            	break;
            case 'estado_time': 
            	$sbHtml = $this->makeByEnd();
            	break;
        }
        
        //se elmina la tabla temporal de actas.
