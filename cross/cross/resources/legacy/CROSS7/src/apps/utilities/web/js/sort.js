...
function selectnone(obj) { /* NEW added from version 1.1 */
	obj = (typeof obj == "string") ? document.getElementById(obj) : obj;
	if (obj.tagName.toLowerCase() != "select" && obj.tagName.toLowerCase() != "select-multiple")
		return;
	for (var i=0; i<obj.length; i++) {
...
function disableNextObject(valor,nextObj)
{
	var nuSize = nextObj.length;
	for(nuCont=0;nuCont<nuSize;nuCont++)
	{
		if(nextObj.options[nuCont].value == valor)
			nextObj.options[nuCont].disabled = true;
		else
			nextObj.options[nuCont].disabled = false;
		nextObj.options[nuCont].selected = false;
	}
	return;
}