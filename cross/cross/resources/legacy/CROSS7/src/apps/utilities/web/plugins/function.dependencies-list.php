...
function smarty_function_dependencies_list($params, & $smarty) {
	...
	$rcFormed = getTransferDependency($orgacodigos);
...
function getDependencies() {
	...
	$objGateway = $objTmp->getGateWay("organizacionExtended");
...
function getTransferDependency($sbOrgacodigos) {
...
*
* Ajusta el tama�o de la memoria de procesamiento de scripts del php
* @author freina<freina@hotmail.com>
* @date 17-Aug-2006 15:50
* @location Cali-Colombia
*/
function setMemoryLimit() {

	settype($sbMemory, "string");
	settype($sbUltimo, "string");
	settype($nuMemory, "integer");

	$sbMemory = ini_get("memory_limit");

	//Hace casting hacia entero
	$nuMemory = (integer) $sbMemory;
	$sbUltimo = $sbMemory {
		strlen($sbMemory) - 1};

	//Si el valor el el php ini es menor a 10M
	switch ($sbUltimo) {
		case 'k' :
		case 'K' :
			if ($nuMemory < 10240)
