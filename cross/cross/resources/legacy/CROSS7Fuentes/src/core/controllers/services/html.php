...
class Html {
	...
	* @param array $rcParams Parametros de visualización (opcional)
	*		["border"] => Borde de la tabla
	*		["cols"] => columnas de la ficha
	*		["size_label"] => Tamaño en % del td de la etiqueta
	*		["size_table"] => Tamaño en % de la tabla
	*		["size_data"] => Tamaño en % de la celda con la data
	*		["size_puntos"] => Tamaño en % de la celda con los :
	...
	* porcentajes de la tabla de acuerdo a el numero de columnas
	*/
	function genCardPub($rcDatos, $rcLabels, $rcParams = array ()) {
		...
		if (!$rcParams["align"])
			$rcParams["align"] = "center";
		if (!$rcParams["size_puntos"])
		    $rcParams["size_puntos"]="3%";
		if(!$rcParams["size_datos"]){
			$rcParams["size_datos"]="";
		}
		//Divide la matriz en las columnas
		...
		foreach ($rcCard as $rcReg) {
			...
			foreach ($rcReg as $key => $value) {
				...
				if (!$rcLabels[$key]["label"] && !$value) {
					...
					$rcHtml[] = "<td valign=top width='".$sbPorcenaje."'>
					                    <table width='".$rcParams["size_data"]."' class='".$rcParams["classtabla"]."' border=0>
					                        <tr><td width='".$rcParams["size_label"]."' valign=top>&nbsp;</td>
					                            <td width='".$rcParams["size_puntos"]."' valign=top>&nbsp;</td>
					                            <td width='".$rcParams["size_datos"]."'>&nbsp;</td>
					                        </tr>
					...
					$rcHtml[] = "<td valign=top width='".$sbPorcenaje."'>
					                    <table width='".$rcParams["size_data"]."' class='".$rcParams["classtabla"]."' border=0>
					                        <tr><td width='".$rcParams["size_label"]."' class='".$rcParams["classlabel"]."' valign=top>".$rcLabels[$key]["label"]."</td>
					                            <td width='".$rcParams["size_puntos"]."' valign=top>".$rcParams["puntos"]."</td>
					                            <td width='".$rcParams["size_datos"]."'>".$value."</td>
					                        </tr>
					                    </table>
