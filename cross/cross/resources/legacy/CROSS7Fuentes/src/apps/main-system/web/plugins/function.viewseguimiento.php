...
*/
function smarty_function_viewseguimiento($params, & $smarty) {
	extract($_REQUEST);
	...
	do {
		...
		$rcDataOrden = $gateway->getByIdOrden($ordenumeros);
		if (!is_array($rcDataOrden)) {
			include ($rcuser["lang"]."/".$rcuser["lang"].".messages.php");
			echo "<script language='javascript'>alert('".$rcmessages[21]." $ordenumeros')\n".
		...
		$rcData = $gateway->getByIdOrdenempresajoin($ordenumeros);
		$rcTmpData = $gateway->getByOrdenOrdenempresa($ordenumeros);
		...
		$dimensionManager->setVadidomivals($ordenumeros);
		...
		if ($result) {
			...
			$rcExtraData = $gateway->getDataFichaAdicional($tmpTable, $ordenumeros, $rcDateFields);
		}
		...
		$rcanexos = $gateway->getByAnexos_fkey($ordenumeros);
		if ($rcanexos) {

			foreach ($rcanexos as $rctmp) {
				$sbname = $ordenumeros."__".$rctmp["anexnombarch"];
		...
		$rcActas = $gateway->getActas($ordenumeros);
		...
		if ($rcData["oremnumsegs"]) {
			$ordenumeros = $rcData["oremnumsegs"];
		} else {
			$ordenumeros = null;
		}
	} while ($ordenumeros);
