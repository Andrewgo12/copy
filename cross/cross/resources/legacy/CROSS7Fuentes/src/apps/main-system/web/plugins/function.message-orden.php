 ...
 */
function smarty_function_message_orden($params, & $smarty) {
	...
	if ($id !== null && $id !== "") {
		...
		if($signal){
			$sbMessage .= "<script language='javascript'> \n";
			$sbMessage .= "if(confirm('$msg')){ \n";
			$sbMessage .="fncopenwindows('FeCrCmdDefaultFichas','topFrame=FeCrCmdDefaultHeadRepoTiemposEjec&mainFrame=FeCrCmdDefaultBodyFichaOrd&ordenumerosFO=".$rcParams[0]."&consult__flag=1+&vars=ordenumerosFO,consult__flag'); \n";
			$sbMessage .= "} \n" ;
			$sbMessage .= "</script> \n";
		}else{
