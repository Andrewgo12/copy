...
class FeCuCmdAddSolicitante {
	function execute() {
		...
		if(isset($sbContacto) && $sbContacto){
			...
			//Se arregla el problema del charset en la interfaz de Solicitante que es dinámica aplicando utf8_encode a lo que viene
			if(!empty($rcSession["contacto"]) && is_array($rcSession["contacto"])){
               $rcSession["contacto"]["contacto__contprinoms"]=utf8_encode($rcSession["contacto"]["contacto__contprinoms"]);
			   $rcSession["contacto"]["contacto__contsegnoms"]=utf8_encode($rcSession["contacto"]["contacto__contsegnoms"]);
			   $rcSession["contacto"]["contacto__contpriapes"]=utf8_encode($rcSession["contacto"]["contacto__contpriapes"]);
			   $rcSession["contacto"]["contacto__contsegapes"]=utf8_encode($rcSession["contacto"]["contacto__contsegapes"]);
			   $rcSession["contacto"]["contacto__contdirecios"]=utf8_encode($rcSession["contacto"]["contacto__contdirecios"]);
		    }
			...
			//Se arregla el problema del charset en la interfaz de Solicitante/Cliente que es dinámica aplicando utf8_encode a lo que viene
			if(!empty($rcSession["cliente"]) && is_array($rcSession["cliente"])){
				$rcSession["cliente"]["cliente__clienombres"]=$rcSession["cliente"]["cliente__clienombres"];
			}
			...
			if($rcTmp && is_array($rcTmp) && $rcTmp["result"]){
				...
				//Se agrega para problema de charset mensaje exitoso solicitante dinámico desde la orden
				$rcResult[1]=$objService->encode(utf8_decode(html_entity_decode($rcmessages[$rcTmp["message"]])));
			}else{
				$rcResult[0] = 0;
				//$rcResult[1]= $objService->encode(html_entity_decode($rcmessages[$rcTmp["message"]]));
				//Se agrega para problema de charset mensaje exitoso solicitante dinámico desde la orden
			...
			//Se agrega para problema de charset mensaje exitoso solicitante dinámico desde la orden //Fher le faltó este
			$rcResult[1]= $objService->encode(utf8_decode(html_entity_decode($rcmessages[0])));
		}
