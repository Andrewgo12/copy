...
Class FeWFCmdAddProceso {
	...
	function execute() {
		...
		if (($proceso__proccodigos != NULL) && ($proceso__proccodigos != "")) {
			WebRequest :: setProperty('cod_message', $sbmessage = 16);
			return "fail";
		}
			...
			(($horas != NULL) && ($horas != "") && ($horas != 0)))) 
		{
			...
			$sbmessage = $proceso_manager->addProceso($proceso__procnombres, $proceso__procdescris, 
			$proceso__perscodigos, $proceso__procestinis, $proceso__procestfins, $proceso__procfeccren, 
			$proceso__orgacodigos, $proceso__proctiempon, $proceso__procactivas, $rcConfig);
