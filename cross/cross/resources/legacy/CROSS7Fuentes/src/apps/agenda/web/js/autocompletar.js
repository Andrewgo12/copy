...
function autoReference(sqlId,params,rcObject,rtnObject,debug){
 ...
 function autoReferenceSelectentes(sqlId,params,rcObject,rtnObject,tiorcodigos){
	homefield = rcObject[0];
	rtnfield = rtnObject;
    var triadas = new Array();
    triadas = params.split("|");
    ...
    params = triadas.join("|");
    if(tiorcodigos.length>0)
    	params += "tiorcodigos="+tiorcodigos;
    //Envia a ejecutar en el RS
    jsrsExecute("rs/jsrs.php", this.fncrtnvalue, "AutoCompletar::autoReference", Array(sqlId,params),false);
 	return ;
 }
 	/**
...
{
	var url = 'index.php?action=FeScCmdLoadIdioma';
...
function reload(req) {
	...
	document.forms[0].langcodigos.value = result;
