...
function smarty_function_select_transference($params, & $smarty) {
	...
	if ($sbFlag) {
		...
		$sbNamedesc = $id."_desc";
		$sbHtml = "<input name='".$name."' type='text' id='".$id."' value=\"".$_REQUEST[$name]."\" onBlur=\"if(this.value!='')autoReference('organizacion','orgacodigos',Array(this),this.form.".$sbNamedesc.");else this.form.".$sbNamedesc.".value='';\">";
		...
		if (!$rcLista) {
			...
			$rcLista = $objService->getActiveOrganizacionByEmployee($rcPersDatos["perscodigos"]);
