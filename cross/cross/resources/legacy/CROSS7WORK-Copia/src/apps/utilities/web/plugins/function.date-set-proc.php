<?php
 /**
	...
	*
	* Despliega un calendario y maneja la fecha según
    * el si el proceso tiene horas configuradas en el parámetro
    * PROC_HOURS
 	* Smarty plugin
	...
	* hour="true"}
*/
function smarty_function_date_set_proc($params, & $smarty) {
	...
	$sbhtml_result = "<input type=text name='$name' size='$size' id='$id'  maxlength='$maxlength' ";
	if ($_REQUEST[$name] !== null) { //Si la fecha viene en el request
        if(is_numeric($_REQUEST[$name])){ //Si esta en formato timestamp
            if($paramHour == true){ // esta definido con horas
