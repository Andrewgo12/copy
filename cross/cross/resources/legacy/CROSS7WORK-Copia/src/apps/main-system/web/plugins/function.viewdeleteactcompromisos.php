...
*/
function smarty_function_viewDeleteActCompromisos($params, & $smarty) {
	extract($_REQUEST);
	extract($params);
	...
	$rcActas = $objGateway->getActas($orden__ordenumeros,$seguimiento);
		...
		foreach ($rcActas as $rcTmpActa) {
			...
			if (is_array($rcAtenc)) {
				...
				foreach ($rcAtenc as $rcTmpAtenc) {
					...
					$rcTmpAtenc["acemnumeros"] = "<a href='#' title=\"".$rclabels_generic["CmdDelete"]."\" onClick=\"".$form.".action.value='".$sbAppId."CmdDeleteActuacionCompromiso';".
					$form.".acta.value='".$sbActacodigos."';".
					$form.".acemnumeros.value='".$sbAcemnumeros."';".
					$form.".orden.value='".$orden__ordenumeros."';".$form.".submit();\" >".$rclabels["delete"]["label"]."</a>";
				...
				//BORRAR LA ULTIMA LINEA DEL RCHTML
				$nuLinea = sizeof($rcHtml);
				$rcHtml[$nuLinea-1] = false;
			}
