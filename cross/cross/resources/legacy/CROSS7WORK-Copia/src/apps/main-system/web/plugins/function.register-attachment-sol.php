 ...
 */
function smarty_function_register_attachment_sol($params, & $smarty) {
	...
	$sbOrdenumeros = $_REQUEST["ordenempresa__ordenumeros"];
	...
	if(!$rcFileName){
		...
		if ($sbOrdenumeros && $_REQUEST["consult"]!=null && $_REQUEST["consult"]!="") {
			...
			WebSession :: setProperty("_rcSolucionFileList", $rcFileName);
			$_REQUEST["consult"] = 1;
		}
	}
	...
	$sbhtml .= "<tr><th colspan='3'><div align='left'>&nbsp;</div></th></tr>";
	//Pinta el textfield tipo file

	$sbhtml .= "<tr>";
	$sbhtml .= "<td width='25%'><B>".$rclabels["nombre"]["label"]."</B></td>";
	$sbhtml .= "<td colspan=\"2\" width='60%'><input type=\"text\" name=\"anexos__archnombres\" id=\"archnombres\" maxlength=\"50\" ".getFieldValidation()."><B>*</B>";
	$sbhtml .= "<tr>";

	$sbhtml .= "<tr>";
	$sbhtml .= "<td width='25%'><B>".$rclabels["tipo"]["label"]."</B></td>";
	$sbhtml .= "<td colspan=\"2\" width='60%'><select id=\"archextensis\" name=\"anexos__archextensis\">".getOption()."</select><B>*</B>";
	$sbhtml .= "<tr>";

	$sbhtml .= "<tr>";
	$sbhtml .= "<td width='25%'>".$rclabels["anexcodigon"]["label"]."</td>";
	$sbhtml .= "<td colspan=\"2\" width='60%'><input type=file name=anexos___anexnombarch size=50 maxlength=100>&nbsp;<a href='#' onClick=\"$form.action.value='FeCrCmdAddAnexosSol';$form.submit();\" >";
	...
	if ($rcFileName) {
		...
		$sbhtml .= "</tr>";
		$sbhtml .= "<td class='titulofila' align='center'>".$rclabels["nombre"]["label"]."</td>";
		$sbhtml .= "<td class='titulofila' align='center'>".$rclabels["tipo"]["label"]."</td>";
		$sbhtml .= "<td class='titulofila' align='center'>".$rclabels["acciones"]["label"]."</td>";
		$sbhtml .= "</tr>"."\n";
		foreach ($rcFileName as $nuCont => $rcTmp) {
			...
			$sbhtml .= "<tr>";
			$sbhtml .="<td align='left' class='$sbEstilo'>".$rcTmp["name"]."</td>";
			$sbhtml .="<td align='left' class='$sbEstilo'>".$rcTmp["ext"]."</td>";
			$sbhtml .="<td align='center' class='$sbEstilo'>"."<a href='#' onClick=\"$form.action.value='FeCrCmdDeleteAnexosSol';$form.deleteattachment.value='".$rcTmp["index"]."';$form.submit();\">";
