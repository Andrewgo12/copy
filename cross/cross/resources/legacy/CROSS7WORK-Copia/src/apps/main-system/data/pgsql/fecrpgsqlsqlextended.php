...
/*			case "tipoorden_web" :
				...
				break;
*/				
//BLOQUE NUEVO CVC CAZAPATA				
			case "tipoorden_web" :
				switch($rcParams[0]){
					case "1":
						$objService = Application :: loadServices("General");
		                $rcTypeClosed = $objService->getParam("cross300", "TIP_CAS_WEB1");
		                if(is_array($rcTypeClosed) && $rcTypeClosed){
		                    $sbTmp = implode("','",$rcTypeClosed);
		                    $sbTmp = ' AND "tiorcodigos" IN (\''.$sbTmp.'\')';
		                }
					break;
					case "2":
						$objService = Application :: loadServices("General");
		                $rcTypeClosed = $objService->getParam("cross300", "TIP_CAS_WEB2");
		                if(is_array($rcTypeClosed) && $rcTypeClosed){
		                    $sbTmp = implode("','",$rcTypeClosed);
		                    $sbTmp = ' AND "tiorcodigos" IN (\''.$sbTmp.'\')';
		                }
					...
					case "3":
						$objService = Application :: loadServices("General");
		                $rcTypeClosed = $objService->getParam("cross300", "TIP_CAS_WEB3");
		                if(is_array($rcTypeClosed) && $rcTypeClosed){
		                    $sbTmp = implode("','",$rcTypeClosed);
		                    $sbTmp = ' AND "tiorcodigos" IN (\''.$sbTmp.'\')';
		                }
					...
					default:
		                ...
		                if(is_array($rcTypeClosed) && $rcTypeClosed){
		                    $sbTmp = implode("','",$rcTypeClosed);
		                    $sbTmp = ' AND "tiorcodigos" NOT IN (\''.$sbTmp.'\')';
		                }
					break;
				}
                
				$sql = 'SELECT * FROM "tipoorden" WHERE "tioractivos"=\''.$sbestado.'\' '.$sbTmp.' ORDER BY "tiornombres"';
				break;
//FIN BLOQUE NUEVO 				
	...
	function getListadoOrdenes($rcParams, $rcParamsDate, $rcView, $rcOrder, $children = null, $tareacc = null) {
		...
		if (is_array($rcParams) && $rcParams) {			
			foreach ($rcParams as $field => $value) {
				...
				$sbLeft = '';
				if($value!=null || $value!=''){
					if($field == 'acta.orgacodigos' && $children){
		...
		//MODIFICACION AGOSTO 29 - PAGINACI�N LISTADO DE CASOS
		//Necesitamos calcular cu�ntos registros hay
