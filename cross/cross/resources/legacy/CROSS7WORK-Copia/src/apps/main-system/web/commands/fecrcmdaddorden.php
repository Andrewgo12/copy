...
class FeCrCmdAddOrden {
	function execute() {
		...
		//|| ($ordenempresa__causcodigos == "" || $ordenempresa__causcodigos == null) 
		...
		//VALICACION DENUNCIA-DENUNCIANTE
		...
		ProvitionalLog::write_log("Customer Type");
		ProvitionalLog::write_log(print_r($_REQUEST['customer_type'], true));
		ProvitionalLog::write_log("Customer identis");
		ProvitionalLog::write_log(print_r($_REQUEST['ordenempresa__contidentis'], true));
		...
		switch($_REQUEST['customer_type']){
			...
			case '2':
				...
				break;
			default:
				ProvitionalLog::write_log("Leave me alone");
				//$_REQUEST['ordenempresa__contidentis'] = $sbdbnull;
				//$_REQUEST['ordenempresa__paciindentis'] = $sbdbnull;
		}
		...
		if (!$sbauto) {
			if (($orden__ordenumeros == "" || $orden__ordenumeros == null)) {
				WebRequest :: setProperty('cod_message', $message = 0);
				WebSession :: setProperty("rcRequest", $_RQUESTTPM);
				return "fail";
			}
		...
		$message = $orden_manager->addOrden($orden__ordenumeros, $rctmp, $rcDimension);
		$_REQUEST["focusposition"] = "";
		if ($message["msg"] == 3) {
			WebRequest :: setProperty('cod_message', $viewmessage = 20);
			$generalService = Application :: loadServices('General');
			$paramHour = $generalService->getParam('workflow', 'PROC_HOURS');
			if ($paramHour == true)
				$message["dateven"] = $serviceDate->fncformatofechahora($message["dateven"]);
			else
				$message["dateven"] = $serviceDate->fncformatofecha($message["dateven"]);
			$sbParams = $message["ordenumeros"].",".$message["dateven"];
			WebRequest :: setProperty('param', $sbParams);
			WebRequest :: setProperty('signal', $sbSignal = true);
			
			//Verifica si la tarea fue asignada a su mismo ente para atenderla
			//Carga el servicio de human_resources
			$rcUser = Application :: getUserParam();
			$objServ = Application :: loadServices("Human_resources");
			
			//Obtiene los entes organizacionales en los cuales el usuario es responsable y los hijos
			$rcPersDatos = $objServ->getPersDatos($rcUser["username"], true);
			$rcEntesPropios = $objServ->getActiveBeingEmployee($rcPersDatos["perscodigos"]);

			if (is_array($rcEntesPropios))
				foreach ($rcEntesPropios as $rcEnte) {
					if ($rcEnte["orgacodigos"] == $message["orgacodigos"]) 
					{
						//Consulta los datos del acta;
						$objServ = Application :: loadServices("Workflow");
						$rcActas = $objServ->getByOrdenActas($message["ordenumeros"]);
						$_REQUEST["acta"] = $rcActas[0]["actacodigos"];
						$_REQUEST["orgacodigos"] = $message["orgacodigos"];
						return "redirect";
					}
				}
			return "success";
	...
	* @date 24-February-2006 09:45:00
