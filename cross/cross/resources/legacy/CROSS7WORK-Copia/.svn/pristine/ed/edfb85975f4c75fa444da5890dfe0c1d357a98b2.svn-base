...
class FeGeArchivosManager{
	function FeGeArchivosManager(){
		...
		$objManager = Application :: getDomainController('NumeradorManager');  //Carga las reglas de los numeradores
		$this->nuAraulinean = $objManager->fncgetByIdNumerador("thread"); //hilo del  proceso. 
		$this->sbStatusP = Application :: getConstant("ARCH_P");   //OK
		$this->sbStatusF = Application :: getConstant("ARCH_F");   //OK
		$this->sbPath = Application :: getConstant("ANEXOS");      //OK 
		$this->objGateway = Application::getDataGateway("archivos");
		$this->objGatewayAux = Application::getDataGateway("archivoaux");   //OK
	...
	function addArchivoaux(){
		...
		if($this->nuAraulinean && $this->nuAchcodigon){
			...
			if(!$this->executeSql){
				...
				$this->objGatewayAux->setSql($rcNull);
			}else{
	...
	function setArchivoaux(){
		...
		if($this->nuAraulinean){
			...
			if(is_array($rcTmp) && $rcTmp){
				//se recorre el arreglo y de acuerdo a la señal se actualiza el archivo en disco y su registro de BD
