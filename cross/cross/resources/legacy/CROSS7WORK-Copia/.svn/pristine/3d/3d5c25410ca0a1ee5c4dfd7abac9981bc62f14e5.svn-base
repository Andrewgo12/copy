...
function LoadSelectEntes(sqlId,params,rcObject,rtnObject,debug)
{
	homefield = rcObject[0];
	rtnfield = rtnObject;
    var rcTriadas = new Array();
    ...
    //Envia a ejecutar en el RS
    jsrsExecute("rs/jsrs.php", this.fncHiglightOptions, "AutoCompletar::getUserSchemaParams", Array(sqlId,params),debug);
 	return ;
}

function LoadSelectPersonal(sqlId,params,rcObject,rtnObject,debug)
{
	homefield = rcObject[0];
	rtnfield = rtnObject;
    var rcTriadas = new Array();
    ...
    //Envia a ejecutar en el RS
    jsrsExecute("rs/jsrs.php", this.fncHiglightOptions, "AutoCompletar::getUserPersonalParams", Array(sqlId,params),false);
 	return ;
}
 
function fncHiglightOptions(sbValue){
	...
	//se carga el combo
 	rcRow = sbValue.split("|");
 	nuCant = rcRow.length;
 	nuTamObj = rtnfield.options.length;
 	if(nuCant>=1)
 	{
	    for(nuCont=0;nuCont<nuTamObj;nuCont++)
	 	{
	 		for(nuAux=0;nuAux<nuCant;nuAux++)
	 		{
	 			if(rcRow[nuAux] == rtnfield.options[nuCont].value)
	 			{
	 				rtnfield.options[nuCont].selected = true;
	 			}
	 		}
	 	}
	}
