...
class FeWFCmdAddActividad {
	function execute() {
		...
		if (($actividad__acticodigos != NULL) && ($actividad__acticodigos != "") && 
			...
			if ($objServ->isDouble($actividad__activalorn) == false) {
				WebRequest :: setProperty('cod_message', $message = 4);
				return "fail";
			}
			...
			$message = $actividad_manager->addActividad($actividad__acticodigos, $actividad__actinombres, $actividad__activalorn, $actividad__actiobservas, $actividad__actiactivas);
