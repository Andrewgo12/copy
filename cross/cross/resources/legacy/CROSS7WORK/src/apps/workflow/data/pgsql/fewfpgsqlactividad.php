...
class FeWFPgsqlActividad {
	var $consult;
	var $objdb;
	function FeWFPgsqlActividad() {
	...
	function addActividad($acticodigos, $actinombres, $activalorn, $actiobservas, $actiactivas) {
		$sql = 'INSERT INTO "actividad" ("acticodigos","actinombres","activalorn","actiobservas","actiactivas")'
		.' VALUES(\''.$acticodigos.'\',\''.$actinombres.'\','.$activalorn.' ,\''.$actiobservas.'\',\''.$actiactivas.'\')';
		$this->objdb->fncadoexecute($sql);
	...
	function updateActividad($acticodigos, $actinombres, $activalorn, $actiobservas, $actiactivas) {
		$sql = 'UPDATE "actividad" SET "actinombres"=\''.$actinombres.'\',"activalorn"='.$activalorn.' ,"actiobservas"=\''.$actiobservas.'\',"actiactivas"=\''.$actiactivas.'\' WHERE "acticodigos"=\''.$acticodigos.'\'';
		$this->objdb->fncadoexecute($sql);
		if (!$this->objdb->objresult)
			$this->consult = false;
		else
	...
	function getAllActividad() {
		$sql = 'SELECT * FROM "actividad" ORDER BY "actinombres"';
		$this->objdb->fncadoselect($sql, FETCH_ASSOC);
		return $this->objdb->rcresult;
	}
