...
function TreeNode(title, icon, link, expanded, isDynamic, js)
{
	this.title     = title;
	this.icon      = icon;
	this.link      = link;
...
function drawMenu()// OPTIONAL ARGS: nodes = [], level = [], prepend = '', expanded = false, visbility = 'inline', parentLayerID = null
{
    ...
    */
	...
	for (var i=0; i<nodes.length; i++) {
        ...
        */
		var divTag    = sprintf('<div class=tdmenu id="%s" style="display: %s; behavior: url(#default#userdata)">', layerID, visibility);
		var onMDown   = doesMenu() && nodes[i].n.length  && nodes[i].isDynamic ? sprintf('onmousedown="%s.toggleBranch(\'%s\', true, \'%s\')" style="cursor: pointer; cursor: hand"', this.myname, layerID,nodes[i].js) : '';
		var imgTag    = sprintf('<img src="%s/%s%s.gif" align="top" border="0" name="img_%s" %s />', this.iconpath, gifname, modifier, layerID, onMDown);
		var linkStart = nodes[i].link ? sprintf('<a class=menuppal href="%s">', nodes[i].link) : '';
...
function toggleBranch(layerID, updateStatus, sbJs) // OPTIONAL ARGS: noSave = false
{
	...
	if (updateStatus) {
        ...
        */
		...
		if(currentDisplay=='inline' && newDisplay=='inline' && sbJs!='null'){
			eval(sbJs);
		}
	}
}
