...
Class FeCuCmdUpdateContacto {
	...
	function execute() {
		...
		&& ($contacto__contindentis != NULL) && ($contacto__contindentis != "") 
		...
		&& ($contacto__grincodigos != NULL) && ($contacto__grincodigos != "")) {
			...
			if ($objServ->formatPrimaryKey($contacto__contcodigon) == false) {
				WebRequest :: setProperty('cod_message', $message = 4);
				return "fail";
			}
			//Hace la validacion de campos numericos y formateo de campos cadena
			if ($objServ->isInteger($contacto__contcodigon) == false) {
				WebRequest :: setProperty('cod_message', $message = 4);
				return "fail";
			}
			...
			$message = $contacto_manager->updateContacto($contacto__contcodigon, 
			$contacto__contindentis, $contacto__tiidcodigos, $contacto__cliecodigon, 
			$contacto__contprinoms, $contacto__contsegnoms, $contacto__contpriapes, 
			$contacto__contsegapes, $contacto__contfecnacis, $contacto__contedadn, $contacto__contsexos, 
			$contacto__contemail, $contacto__locacodigos, $contacto__contdirecios, 
			$contacto__conttelefons, $contacto__contobservs, $contacto__contactivas,$contacto__contnumcels,$contacto__grincodigos);
