...
class FeCrCmdAddOrdenWeb {
	function execute() {
		...
		if ($message["msg"] == 3) {
			...
			$_REQUEST["ordenempresa__orgacodigos"] = $ordenempresa__orgacodigos;
			$_REQUEST["orden__ordefecregd"] = $serviceDate->fncformatofechahora($serviceDate->fncintdatehour() );
			$_REQUEST["ordenempresa__merecodigos"] = $ordenempresa__merecodigos;
			return "success";
		} else {
			$_REQUEST = $_RQUESTTPM;
			WebRequest :: setProperty('cod_message', $message);
