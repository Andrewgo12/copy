...
function smarty_function_viewfichaord($params, & $smarty) {
	...
	// Bypass para acceso sin sesión
	if (!is_array($rcuser)) {
		$rcuser["lang"] = "es";
		$rcuser["username"] = "public";
	}
	...
	if (!is_array($rcDataOrden)){
		// Mensaje simplificado sin JavaScript
		return "<div style='text-align:center; color:red; font-weight:bold;'>No se encontró información para el número: $orden__ordenumeros</div>";
	}
...
function getDescLang($sbTatlnomtabls,$sbTatlvalcods,$rcTemplate){
	...
	if($sbTatlnomtabls && $sbTatlvalcods && $rcTemplate && is_array($rcTemplate)){
		...
		if (!is_array($rcUser)) {
			...
			$rcUser["lang"] = "es"; // Forzar español por defecto
		}
