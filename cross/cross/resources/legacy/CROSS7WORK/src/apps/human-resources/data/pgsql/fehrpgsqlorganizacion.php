...
class FeHrPgsqlOrganizacion {
	
	function FeHrPgsqlOrganizacion() {
	...
	function addOrganizacion($orgacodigos, $organombres, $tiorcodigos, $orgacgpads, $orgaordenn, $orgafechcred, $esorcodigos, $grupcodigos, $orgatelefo1s, $orgatelefo2s, $locacodigos) {
		...
		$sbSql = 'INSERT INTO "organizacion" ("orgacodigos","organombres","tiorcodigos","orgacgpads","orgaordenn","orgafechcred","esorcodigos","grupcodigos","orgatelefo1s","orgatelefo2s","locacodigos")'
	...
	function getAllOrganizacion() {
		$sql = 'SELECT * FROM "organizacion" ORDER BY "organombres"';
		$this->objdb->fncadoselect($sql, FETCH_ASSOC);
		return $this->objdb->rcresult;
	}
	...
	function getAllOrganizacionById() {
		$sql = 'SELECT * FROM "organizacion" ORDER BY "organombres"';
		$this->objdb->fncadoselect($sql, FETCH_ASSOC);
		$rcOrga = $this->objdb->rcresult;
		if(!is_array($rcOrga))
		return false;
		foreach($rcOrga as $row) {
			$rcResult[$row["orgacodigos"]] = $row;
		}
		return $rcResult;
	}
