...
function smarty_function_viewReportManagement($params, & $smarty) 
{
	...
	if ($ordefecdiginin && $ordefecdigfinn) {
		$ordefecdiginin = $objDate->fncdatehourtoint($ordefecdiginin);
		$ordefecdigfinn = $objDate->fncdatehourtoint($ordefecdigfinn);
  	...
  	switch ($report) {
   		case 14:
    		...
    		if($tiorcodigos) {
    			$reportesManager->gateway->setTypeCase($tiorcodigos);
    			$tiornombres = getAditionalData("tipoorden",$tiorcodigos,"tiornombres");
    		}
   		...
   		case 15:
			...
			$reportesManager->getCasesByMunicipio();
       	break;
       	case 16:
       		//4 Cantidad de denuncias por localización
       		$reportesManager->setStartDate($ini_date);
  			$reportesManager->setEndDate($fin_date);
  			
  			//obtenemos el tipo caso denuncia
  			$objGeneral = Application::loadServices("General");
  			$tiorcodigos = $objGeneral->getParam("cross300","DENUNCIA_TC");
  			if($tiorcodigos) {
    			$reportesManager->gateway->setTypeCase($tiorcodigos[0]);
    		}
			$reportesManager->getDenunciasPorLocaliza();
       	break;
       	case 18:
       	...
       	case 19:
       		...
       		$reportesManager->setStartDate($ini_date);
  			$reportesManager->setEndDate($fin_date);
       		$reportesManager->getClientSatisfactionIndicator();
       	break;
		case 20:
       		//11 Indicador de casos de soporte atendido
       		$reportesManager->setStartDate($ini_date);
