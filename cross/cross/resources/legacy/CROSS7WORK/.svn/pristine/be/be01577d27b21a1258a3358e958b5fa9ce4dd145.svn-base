...
class FeGeAutoCompletar {
	...
	*/
	...
	function getUserSchemaParams($isbSqlId,$isbParams)
	{
		settype($osbReturn,"string");
		...
		$rcAll = $objParams->getAllParams($nuSchema);
		
		if(array_key_exists($sbModule,$rcAll))
			if(array_key_exists($sbParam,$rcAll[$sbModule]))
				if(array_key_exists($sbUser,$rcAll[$sbModule][$sbParam]))
					$rcGrants = $rcAll[$sbModule][$sbParam][$sbUser];
					
		if($rcGrants)
			$osbReturn = join("|",$rcGrants);
		return $osbReturn;
	}
	
	function getUserPersonalParams($isbSqlId,$isbParams)
	{
		settype($osbReturn,"string");
		settype($sbUser,"string");
		settype($sbModule,"string");
