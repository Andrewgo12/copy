 ...
 */
function smarty_function_drawSolicitantePub($params, & $smarty) {
	...
	include_once ("config/config.solicitantepub.php");
	...
	if($rcSession && is_array($rcSession)){
		...
		$rcParams["classlabel"]= "tdlabels";   //Clase de la celda con lables de tabla dinámica
		$rcParams["classtabla"]= "tablepub";   //Clase de la tabla dinámica de cada campo
		$rcParams["puntos"]= ":";         //Caracter que separa el label del dato
		$rcParams["border"]=0;
		$rcParams["size_label"]="200";    //Tamaño relatico de celda con labels
		$rcParams["size_table"]="998";   //Tamaño de la tabla
		$rcParams["size_data"]="498px";  //Tamaño fijo de la tabla
		$rcParams["size_puntos"]="2px";    //Tamaño relativo de celda de :
		$rcParams["size_datos"]="";    //Tamaño relativo de celda con los datos
	
		$sbHtml1 = $objService->genCardPub($rcField1[1], $rcField1[0], $rcParams);

		if($rcSession["signal"]==1){
			$sbHtml = "<table align=\"center\" border=\"0\" width=\"998px\" class=\"".$rcParams["classtabla"]."\">";
			$sbHtml .= "<tr>";
			...
			if($rcSession["signal"]==2){
				...
				$sbHtml2 = $objService->genCardPub($rcField2[1], $rcField2[0], $rcParams);
				$sbHtml = "<table align=\"center\" border=\"0\" width=\"998px\">";
				$sbHtml .= "<tr>";
				$sbHtml .= "<td align=\"center\" class='titulofila'>";
