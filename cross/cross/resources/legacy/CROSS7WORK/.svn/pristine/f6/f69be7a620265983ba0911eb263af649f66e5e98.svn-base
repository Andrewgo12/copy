...
class FeGeCmdDrawListTablastipole {
	function execute() {
		...
		if($entidad && $langcodigos){
			
			//se arma el arreglo de parametros
			$rcParams["entidad"] = $entidad;
			$rcParams["langcodigos"] = $langcodigos;
	...
	function drawList($rcParams){
		...
		$sbPath = Application::getPluginsDirectory()."/function.drawListTablastipole.php";
		include($sbPath);
		$sbHtml = smarty_function_drawListTablastipole($rcParams,$this,false);
		return $sbHtml;
	}
}
?>