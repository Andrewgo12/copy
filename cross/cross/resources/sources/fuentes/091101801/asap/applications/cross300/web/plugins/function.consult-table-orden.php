...
function smarty_function_consult_table_orden($params, & $smarty) {
	...
	if (!$_REQUEST['sqlConsult']) {
		...
		if(is_array($rcFields)){
			...
			if($rcFields["ordenempresa.contidentis_p"]){
                $rcFields["ordenempresa.contidentis"] = $rcFields["ordenempresa.contidentis_p"];
                 unset($rcFields["ordenempresa.contidentis_p"]);
            }
			
			foreach ($rcFields as $key => $value){
				if(!(strpos($key, "anexos")===false)){
					unset($rcFields[$key]);
				}	
			}	
		}
