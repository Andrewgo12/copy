	...
	*/
	function addSchema($sbSchenombres, $sbScheobservas) {
		...
		if ($rcData && is_array($rcData)) {
			...
			$nuSchecodigon = $objNumerador->fncgetByIdNumerador('schema');
			
			//Primero, se crea el schema en la bd, luego se mueven serializados
			$nuResult = $this->gateway->createSchema($nuSchecodigon);
			if($nuResult)
			{
				$this->gateway->getSqlUseSchema("profiles");
				
				$sbResult = $this->gateway->addSchema($nuSchecodigon, $sbSchenombres, $rcData[0]["schedbusers"], $rcData[0]["schedbkeys"], $sbScheobservas);
				if ($sbResult)
				{
