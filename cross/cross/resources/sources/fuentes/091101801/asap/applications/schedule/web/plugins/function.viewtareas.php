...
function smarty_function_viewtareas($params, & $smarty) {
	...
	$rcTareas = $objService->getAsignWork($orgacodigos,$order_by,$$order_by);
	...
	$rcHtml[] =  "<tr><th colspan='$nuCols'><div align='left'>".$rclabels["tareas"]["label"]."</div></th></tr>";
	$rcHtml[] =  "<tr>";
	//Trae las etiquetas
	include ($rcUser["lang"]."/".$rcUser["lang"].".acta.php");
	//Pinta las cabeceras
	foreach ($rcTareas[0] as $sbKey => $sbValue){
        if($sbKey != 'actacodigos'){
            $rcHtml[] =  "<td class='titulofila'>";
            ...
            $rcHtml[] =  "   <input type='hidden' name='$sbKey' value='$sentido'>";
            $rcHtml[] =  "</td>";
        }
    }
	...
	foreach ($rcTareas as $nuIndex => $rcActa) {
		...
		$rcHtml[] =  "<td class='$sbEstilo'>";
		if ($sbIsOwner == true) {
			$rcHtml[] =  "<a href='#' title=\"".$rclabels_generic["reg_a"]."\" onClick=\"$form.action.value='".$sbAppId."CmdDefaultActaempresa';$form.acta.value='".$rcActa["actacodigos"]."';$form.submit();\" ><img src='$sbImgDir/editar.gif' border='0' title=\"".$rclabels_generic["reg_a"]."\" align='absmiddle'></a>";
		}
		$rcHtml[] =  "<a href='#' title=\"".$rclabels_generic["trans"]."\" onClick=\"$form.action.value='".$sbAppId."CmdDefaultTransferTareas';$form.acta.value='".$rcActa["actacodigos"]."';$form.submit();\" ><img src='$sbImgDir/actualizar_002.gif' border='0' title=\"".$rclabels_generic["trans"]."\" align='absmiddle'></a>";
		$rcHtml[] =  "<a href='#' title=\"".$rclabels_generic["view_t"]."\" onClick=\"javascript:fncopenwindows('".$sbAppId."CmdDefaultFichas','topFrame=".$sbAppId."CmdDefaultHeadFicha&mainFrame=".$sbAppId."CmdDefaultFichaActa&acta=".$rcActa["actacodigos"]."&vars=acta');\" ><img src='$sbImgDir/consultar_002.gif' border='0' title=\"".$rclabels_generic["view_t"]."\" align='absmiddle'></a>";
		$rcHtml[] =  "<a href='#' title=\"".$rclabels_generic["view_r"]."\" onClick=\"javascript:fncopenwindows('".$sbAppId."CmdDefaultFichas','topFrame=".$sbAppId."CmdDefaultHeadRepoTiemposEjec&mainFrame=".$sbAppId."CmdDefaultBodyFichaOrd&ordenumerosFO=".$rcActa["ordenumeros"]."&vars=ordenumerosFO');\" ><img src='$sbImgDir/zoomprev.gif' border='0' title=\"".$rclabels_generic["view_r"]."\" align='absmiddle'></a></td></tr>";
	}
	$rcHtml[] =  "</tr>";
	$rcHtml[] =  "</table>";
    return implode("\n",$rcHtml);
